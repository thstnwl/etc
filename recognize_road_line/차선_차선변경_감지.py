...
def DetectRoadLine(src):
    ...
    #rectangle = np.array([[(0, canny.shape[0]), (240, 240), (400, 240), (640, canny.shape[0])]])  # 검출 구역 수정 필요
    rectangle = np.array([[(0, canny.shape[0]+50), (260, 220), (380, 220), (590, canny.shape[0])]])

    # 크기가 can과 동일한 검은색 배경 mask 생성
    mask = np.zeros_like(canny)

    # mask의 rectangle 영역을 흰색으로 채움     
...
cap = cv2.VideoCapture('drive.avi')   # 영상 로드
...
while cap.isOpened():
    ...
    if abs(dgree_L) <= 155 or abs(dgree_R) <= 155:
        if dgree_L == 0 or dgree_R == 0:
            state = 'Left' if (dgree_L < 0 or dgree_R < 0) else ('Right' if(dgree_L > 0 or dgree_R > 0) else '')
        elif abs(dgree_L - 15) > abs(dgree_R):
            state = 'Right'
        elif abs(dgree_R + 15) > abs(dgree_L):
            state = 'Left'
        #else:
            #state = 'Go'
    else:
        if dgree_L > 155 or dgree_R > 155:
            state = 'hard right'
        elif dgree_L < -155 or dgree_R < -155:
            state = 'hard left'
    #print(state)
